...
function FinalThoughts({ report }: { report: Report }) {
  return (
    <div className="rounded-xl border border-[rgba(178,178,178,0.40)] bg-[#F8FAFF] overflow-hidden">
      {/* Header bar */}
      <div className="flex items-center gap-2 px-6 py-3 bg-[#EFF6FF] border-b border-[rgba(178,178,178,0.40)]">
        <ChatIcon />
        <span className="font-inter text-sm font-medium text-[#0171F9]">FINAL THOUGHTS</span>
      </div>
      ...
      <div className="px-6 py-4 flex flex-col gap-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1.5">
            <SchoolIcon />
            <span className="text-[#0F1724] font-[Outfit] text-md font-medium">Would you return to this school?</span>
          </div>
        ...
        <div className="flex items-center gap-2.5 pl-5">
          ...
          <p className="text-[#464555] font-inter text-[15px] italic font-normal leading-5">{report.returnToSchoolQuote}</p>
        </div>
      </div>
      ...
      <div className="px-6 pt-4 pb-5">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-1.5">
            <UserIcon />
            <span className="text-[#0F1724] font-[Outfit] text-md font-medium">Would you return for this teacher or class?</span>
          </div>
          <ReturnPill answer={report.returnToTeacher} />
        </div>
        <div className="flex items-center gap-2.5 pl-5">
          <div className="min-h-[30px] w-[3px] self-stretch bg-[#22C55E] flex-shrink-0 rounded-full" />
          <p className="text-[#464555] font-inter text-[15px] italic font-normal leading-5">{report.returnToTeacherQuote}</p>
        </div>
      </div>
    </div>
  );
...
function ReportCard({ report, defaultExpanded = false }: { report: Report; defaultExpanded?: boolean }) {
  ...
  return (
    <div className={`bg-white rounded-xl shadow-[0_4px_6px_-1px_rgba(0,0,0,0.10)] ${showRatings ? "p-7" : "px-7 pt-7"} flex flex-col gap-6`}>
      {/* Header row */}
      <div className="flex flex-col sm:flex-row sm:items-start gap-3 sm:gap-3">
        {/* Left: school info */}
        <div className="flex flex-col gap-3 flex-1">
          <h2 className="font-inter text-base font-bold text-[#121212]">{report.school}</h2>
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex flex-wrap items-center gap-3">
              <div className="flex items-center gap-1.5 opacity-80">
                <LocationIcon />
                <span className="font-[Outfit] text-xs text-[#414141]">{report.location}</span>
              </div>
              ...
              <div className="flex items-center gap-1.5">
                <TeacherIcon />
                <span className="font-inter text-xs font-medium text-[#121212] opacity-70">{report.teacher}</span>
              </div>

              <span className="w-[3px] h-[3px] rounded-full bg-[#676767] opacity-64" />

              <span className="px-2 py-1 rounded text-xs font-semibold font-inter bg-[#DFEEFF] text-[#0171F9]">
                {report.grade}
              </span>
            </div>
            ...
            <div className="flex items-center gap-1.5 opacity-80">
              <CalendarIcon />
              <span className="font-[Outfit] text-xs text-[#414141]">{report.date}</span>
            </div>
          </div>
        </div>
      ...
      {/* Body text */}
      <p className="font-inter text-base text-black opacity-70 leading-7">{report.body}</p>
      ...
      <div className="flex flex-wrap items-center gap-3">
        {report.tags.map((tag) => (
          <span
            key={tag}
            className="px-2.5 py-2 rounded-md border border-[#EAEAEA] bg-[#F2F2F2] text-[#9A9A9A] font-inter text-sm font-medium leading-[15px]"
          >
            {tag}
          </span>
        ))}
    ...
    <div className="flex items-center justify-center gap-2 pt-4">
      ...
      {pages.map((p) => (
        <button
          ...
          className={`w-9 h-9 flex items-center justify-center rounded-md text-sm font-medium font-inter transition-colors ${
            current === p
...
export default function MyReportsPage() {
  return (
    <div className="min-h-screen flex flex-col bg-[#F8FAFE]">
      ...
      <div className="bg-white">
        <div className="max-w-[1440px] mx-auto px-6 sm:px-14">
          ...
          <div className="pt-[72px] pb-8">
            <h1 className="text-[#121212] font-[Inter] text-[clamp(32px,5vw,48px)] font-bold leading-[1.2]">My Reports</h1>
          ...
          <div className="flex items-center gap-2 bg-[#E9F2FF] rounded-2xl p-2">
            <div className="flex items-center gap-2 flex-1 h-[54px] bg-white rounded-lg border border-[rgba(195,198,214,0.20)] px-4">
              <SearchIcon />
              <input
                type="text"
      ...
      {/* Reports list */}
      <main className="flex-1 max-w-[1440px] mx-auto w-full px-6 sm:px-14 py-8 pb-[80px] flex flex-col gap-8">
